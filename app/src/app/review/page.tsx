...
export default async function ReviewPage({
  ...
  if (topicSlug) {
    const [t] = await db.select().from(topics).where(eq(topics.slug, topicSlug)).limit(1);
    if (t) {
      topicFilterId = t.id;
      topicTitle = t.title;
    }
  ...
  const whereClause = topicFilterId
    ...
    : and(eq(cards.status, "active"), dueClause);
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 space-y-6">
      <div className="flex items-center justify-between">
        <Link
          href={topicSlug ? `/topics/${topicSlug}` : "/topics"}
      ...
      <ReviewSession cards={queue} topicSlug={topicSlug} />
    </div>
  );
}
