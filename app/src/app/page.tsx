...
export default async function HomePage() {
  ...
  const [topicCount] = await db.select({ n: count() }).from(topics);
  const [questionCount] = await db.select({ n: count() }).from(questions);
  const [activeCardCount] = await db.select({ n: count() }).from(cards).where(eq(cards.status, "active"));
  const [dueCount] = await db
    .select({ n: count() })
    .from(cards)
    .where(and(eq(cards.status, "active"), lte(cards.dueAt, now)));
  ...
  const recentTopics = await db
    .select()
    .from(topics)
    .where(isNotNull(topics.lastVisitedAt))
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 space-y-8">
      <header>
        <h1 className="text-3xl font-bold tracking-tight">System Design Lab</h1>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            <p className="text-sm text-muted-foreground">topics · {questionCount.n} design questions</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            <p className="text-sm text-muted-foreground">
              active · {pendingCount.n > 0 && <Badge variant="muted" className="ml-1">{pendingCount.n} pending review</Badge>}
            </p>
            {pendingCount.n > 0 && (
              <Button asChild variant="ghost" size="sm" className="px-0 h-auto">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <ul className="space-y-2">
                {recentTopics.map((t) => (
                  <li key={t.id} className="flex justify-between items-center text-sm">
                    <Link href={`/topics/${t.slug}`} className="hover:underline">{t.title}</Link>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <MessageSquare className="h-4 w-4" /> Recent mock interviews
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <ul className="space-y-2">
                {recentSessions.map((s) => (
                  <li key={s.id} className="flex justify-between items-center text-sm">
                    <Link href={`/interview/sessions/${s.id}`} className="hover:underline">
                      Session #{s.id}
                    </Link>
